    ...
    <div class="escolher-plano">
        <div class="hospedagem-info">
            ...
            <div class="valor-monetario">
                <span v-if="preco == 0"><strong>Grátis</strong></span>
                <span v-if="preco > 0">
                    R$<strong>{{ preco > 0 ? preco : 'Grátis' }}</strong>/mes
                </span>
            </div>
            <div>{{ preco > 0 ? condicao : 'Você não paga nada para usar'}}</div>
            <div>sem taxa de setup</div>
            <hr>
            <div>{{preco > 0 ? siteMaisVisita : 'Ideal para quem está começando.'}}</div>
            <hr>
        </div>
        <button class="escolher-plano-button">ESCOLHER ESSE PLANO</button>
        <div>
            <h3>Seu site em servidores <u>no {{ preco > 0 ? paisServidor : 'Estados Unidos' }}</u></h3>
            <ul class="lista-conteudo" v-if="preco == 0">
                <li v-for="(item, index) in listaDeServidoresGratis" :key="index">{{ item }}</li>
            </ul>
            <ul class="lista-conteudo" v-if="preco > 0 && preco < 999">
                <li v-for="(item, index) in listaDeServidores499" :key="index">{{ item }}</li>
            </ul>
            <ul class="lista-conteudo" v-if="preco > 499">
                <li v-for="(item, index) in listaDeServidores999" :key="index">{{ item }}</li>
            </ul>
            <h3>Suporte 24 horas, 7 dias por semana grátis;</h3>
            <h3>Aplicativos disponíveis:</h3>
            <ul class="lista-conteudo">
                <li v-for="(item, index) in listaDeAplicativos" :key="index">{{ item }}</li>
            </ul>
            <h3  v-if="preco > 0">Migração gratuita;</h3>
            <ul class="lista-conteudo" v-if="preco > 0">
                <li v-for="(item, index) in migracaoGratuita" :key="index">{{ item }}</li>
            </ul>
